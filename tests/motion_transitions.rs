use billiards::{
    ...
    InchesPerSecondSq, MotionPhase, MotionPhaseConfig, MotionTransitionConfig, RadiansPerSecondSq,
...
fn a_resting_ball_has_no_next_transition() {
    ...
    assert!(
        compute_next_transition(&state, &BallSetPhysicsSpec::default(), &transition_config())
            .is_none()
    );
}
