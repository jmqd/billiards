...
AUTHORITY_MANIFEST_PATH = WHITEPAPERS_DIR / "authority_manifest.json"
...
def load_authority_manifest() -> dict[str, dict]:
    try:
        ...
        raise SystemExit(f"Missing authority manifest: {AUTHORITY_MANIFEST_PATH}") from error
...
- `whitepapers_index.jsonl`
  - one JSON object per retrieval-eligible document in `whitepapers/`
  - includes title, topics, citation status, reviewed retrieval policy, and short excerpt
- `whitepapers_corpus.txt`
...
Indexed documents: {len(records)} retrieval-eligible files selected by
`whitepapers/authority_manifest.json`.
...
def main() -> None:
    ...
    policies = load_authority_manifest()
    ...
    for path in iter_docs(policies):
