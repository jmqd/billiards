...
### P0 — Implement TP B.10 table-bounce tangential impulse and spin loss
...
### P0 — Schedule airborne ball-ball, rail, jaw, and pocket interactions before table contact

**Problem.** Airborne balls are excluded from all interactions except their next table contact. That makes jump/elevated shots unable to hit an object ball while airborne, hit a rail/jaw while airborne, or foul/scratch through an airborne path.

- Current code clears jaw, pocket, rail, transition, and ball-ball predictions for `NBallSystemState::Airborne`: `src/lib.rs:1831-1842`, `src/lib.rs:1961-1976`, `src/lib.rs:2085-2110`.
- Event advancement simply advances airborne states ballistically until the selected event: `src/lib.rs:10073-10096`.
...
**Target.** Add 3D-aware airborne event prediction at least for cue-ball/object-ball contact and table-boundary/pocket interactions. Until full 3D collision response exists, surface explicit diagnostics instead of silently ignoring pre-landing contacts.

### P1 — Use TP B.2 rolling side-spin turn in actual motion, not only metadata

**Problem.** The TP B.2 turn estimator is correct as analysis data, but rolling motion integration still advances in a straight line. Consumers of `advance_motion_on_table`, trace playback, rail prediction, and future ball-collision prediction do not follow the estimated curved path.

- Straight-line rolling integrator: `src/lib.rs:4124-4171`.
- Comment explicitly says TP B.2 is report-only: `src/lib.rs:11449-11464`.
...
**Target.** Either integrate TP B.2 curvature into rolling advancement/scheduling, or explicitly split the API into `physics_state` versus `analysis_annotation` so callers cannot mistake the estimator for simulated motion.
...
### P2 — Source-calibrate vertical launch and rebound coefficients
